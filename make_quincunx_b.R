...
all.circles = vector(mode = "list", length = 4)

for (sx in 1:length(all.circles)) {
  set.seed(sx)
  ...
  all.circles[[sx]] = circles
}

x0 <- seq(-60,60,len=100)


plot_quincunx = function(circles) {
  p <- ggplot(circles[t == i]) +
    geom_point(aes(x, y, col = col), size = .75) +
    geom_segment(data = segments_df, aes(x = x1, y = y1, xend = x2, yend = y2)) +
    coord_fixed() +
...
for (i in 1:Nt) {
  ...
  p = 
    plot_quincunx(all.circles[[1]]) +
    plot_quincunx(all.circles[[2]]) +
    plot_quincunx(all.circles[[3]]) +
    plot_quincunx(all.circles[[4]]) + 
    plot_layout(design = layout)
  
  outfile <- sprintf("quincunx/%04i.png", i)
  ggsave(outfile, p, width = 9, height = 9)
}
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
root_name <- "galton"
mp4_name  <- paste0(root_name, "_pane", ".mp4")
