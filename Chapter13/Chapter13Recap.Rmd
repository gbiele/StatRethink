...
```{r class.source = 'fold-show'}
precis(fit.full_pooling) %>% round(2)
precis(fit.full_pooling.b,depth = 2) %>% round(2)
```

Do the two models really describe the data equally well, and is the number of effective parameters more or less equal? We use the `compare` function to check this.

```{r class.source = 'fold-show'}
compare(fit.full_pooling, fit.full_pooling.b) %>% round(2)
```
...
As expected we estimated the same mean for department.
...
__Getting from a full-pooling to a no-pooling model by changing a model parameter shows that this are not qualitatively distinct models, but opposite endpoints of a continuous model space.__
...
m.no_pooling.b = alist(
  S ~ dnorm(mu, sigma),
  mu <- mu_bar + z[d]*1000, # if we remove the *1000 here
  z[d] ~ dnorm(0,1),        # this becomes dnorm(0,1000)
  mu_bar ~ dnorm(3,3),
  sigma ~ dexp(2),
...
Here we compare the parameter fits of the standard form of the no-pooling model and of the non-standard form. the tiny crosses are MCMC standard errors, and the fact that the line lies within the standard error tells us the that the difference of the estimated parameters is smaller than random variation due to MCMC sampling (of the same model).
...
<table>
  ...
  <tr>
    ...
    $$S_i \sim normal(\mu_d, \sigma)\\ {\mu_d} \sim normal(\bar \mu,\color{red}{\sigma_{\mu}}) \\ \sigma \sim exponential(1) \\ \bar \mu \sim normal(3,3) \\ \color{red}{\sigma_{\mu} \sim halfnormal(0,2)}$$
...
Here is a plot of these absolute deviations:
...
check.accuracy = function(sd_dep_satisfaction = .25, n_departments = 15) {
  fn = paste0("fits/accuracy_",100*sd_dep_satisfaction,"_",n_departments,".Rdata")
...
Here, all models are doing well (check the y-axis).
...
```{r, fig.width=7, fig.height=4, out.width="110%"}
set_par(cex = 1, mar=c(3,3,3,.5))
check.accuracy(sd_dep_satisfaction = 15, n_departments = n_depts)
```
...
```{r, fig.width=5.5, fig.height=5, out.width="50%"}
VB.partial_pooling = 
...
\color{red}{\sigma_\alpha \sim Exponential(1)}
$$
...
To learn about posterior distributions, always start by generating posterior distributions, and that group and plot the data according to the question. For instance, if we are interested in variation across ponds, we make predictions for tanks and calculate variances over tanks

```{r eval = FALSE, class.source = 'fold-show'}
# get model predictions
preds = sim(model_list[[m]])
# calculate mean probability for each tank
p = colMeans(preds)/dat$N
```

```{r eval = FALSE, class.source = 'fold-show'}
model_list <- 
  list(m13.m1_0, m13.m1_pred, m13.m1_size, m13.m1_pred_size, m13.m1_pred_size_interact)


par (mar=c(2,2,2,1), mgp=c(2,.2,0), tck=-.01,mfrow = c(2,3))
for (m in 1:length(model_list)) {
  ...
  plot(1:length(dat$N),
       p,
       ylim = c(0,1), xlim = c(0,length(dat$N)+1),
     ylab = "probability", xlab = "tank")
  # print sd over tanks
  text(15,.1, paste("sd =", round(sd(p),3)))
  title(model_list[[m]]@formula[[2]], cex.main = .5)
}
```
This simple plot only shows the averages over samples. A more complicated plot could show the posterior distribution for instead of averages.
