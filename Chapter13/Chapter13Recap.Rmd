...
- The last years have brought many changes to the firm, whereby all departments were typically impacted in a similar manner by these changes. Therefore you think that employee satisfaction should be similar in the departments.
...
__Models in which we estimate the between group standard deviation are called partial pooling models, because they lie somewhere on the continuum between no and full polling.__
...
One _issue_ with multilevel models (with partial pooling) is that they are not "unbiased", which means that they do not provide an estimate of the sample mean.
...
- Hierarchical models assume estimate group level parameters by assuming that these come from a common distribution
