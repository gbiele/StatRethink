...
for (k in 1:nrow(params)) {
  mu = params$mu[k]
  sd = params$sd[k]
  params$UP[k] = prod(dnorm(age,mu,sd)) * dnorm(mu,25,2) * dexp(sd,0.5)
}
```
...
# probabilities "PP"
library(ggplot2)
ggplot(params, aes(x = mu, y = sd, fill = P)) + geom_raster()
```


## Grid approximation, version 2

Here is a template to generate a grid of parameters. Your main task here is to add the code to calculate the unnormalaized posterior.
You have to set `eval = TRUE` at the beginning of the posterior so that the code is run when you compile.
...
for (i in 1:N_grid_points)
  ...
  {
    posterior_unstandardized[i,j]<- # yourcode comes here
  }
```
...
for (i in 1:N_grid_points)
  for (j in 1:N_grid_points)
  {
    posterior_unstandardized[i,j] <- dnorm(my_mu[i],25,2)*dexp(my_sigma[j],0.5)*prod(dnorm(age,mean=my_mu[i], sd=my_sigma[j]))
  }
