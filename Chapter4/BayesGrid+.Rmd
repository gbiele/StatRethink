...
for (k in 1:nrow(params)) {
  ...
  mu = alpha + beta*my_data$age.c
  params$UP[k] = prod(dnorm(my_data$time,mu,sigma)*dnorm(alpha,0,3)*dnorm(beta,0,2)*dnorm(sigma,0,2))
}
```
...
smoothScatter(posterior$alpha, posterior$beta)
smoothScatter(posterior$alpha, posterior$sigma)
smoothScatter(posterior$beta, posterior$sigma)
```
