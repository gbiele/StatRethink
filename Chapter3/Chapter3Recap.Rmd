...
In Bayesian statistics, we use probability distribution functions to represent uncertainty.

Before we display uncertainty, let's look at how it function looks if we are certain to land on water. The x axis of the function goes from 0 to 1, because proportions are limited to this range of numbers.

```{r, echo = F}
curve(dbeta(x,100000,1),
      n = 1000,
      ylab = "density",
      xlab = "p",
      col = "blue",
...
If we are certain to land on water, $P(water)$ = 1^[The figure shows density, not probability] and all other probabilities have the value zero. Note that density is not the same as probability.

If we want to express uncertainty, our function has to allow all possible values (and forbid all impossible values) on the x-axis. If we had no information whatsoever to say something about the probability to land on water, all probabilities p get the same value.
...
abline(v = CI80, col = "lightblue", lwd = 2, lty = 3)
