...
$$
\begin{align*}
OR = & \frac{\textrm{failure odds no prior fail}}{\textrm{failure odds prior fail}} \\
...
- the regression weight failures is `r b1`, which means that the odds ratio and approximate relative risk to fail the class if one has failed it once before is exp(`r b1`) `r exp(b1)`. 
- the regression weight maternal education is `r b2`, which means that the odds ratio and approximate relative risk to fail the class if one has failed it once before is exp(`r b2`) = `r exp(b2)`. Correspondingly, if maternal education is changed by two levels, the OR (~RR) is exp(2 * `r b2`) = `r exp(2*b2)`
