...
To see the ordered logistic regression in action, we'll use the example data from the Portuguese school again. In particular, we are looking at the (coarsened) first grade and estimate again the association with maternal education.
...
df$G3 %>% table() %>% barplot()
```
Yes, ordinal logistic regression can also handle multi-modal data!
...
Now we can plot the simple thresholds calculated manually against those estimated with ulam:
...
$$
\delta = \frac{1}{3}\sum^i_{1:3} G(fails = i) - G3(fails = i-1)
$$
...
- the basic thresholds are just logg-odds ratios of probability to respond at levels 1 to k vs k+1 to max level.
- regression weights can be understood as shift of the location of the latent variable, or as shift of all thresholds. The letter interpretation makes clear why they can be interpreted as log odds ratio.
- To understand the results of ordinal logistic regressions it is most useful to generate posterior predictions (using thresholds and category values) and the to calculate contrasts of interest.