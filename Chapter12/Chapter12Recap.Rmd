...
$$
\textrm{log odds ratio} = log \left( \frac{P_{fail}}{P_{pass}} \right) = log \left( \frac{P_{fail}}{1-P_{fail}} \right)
$$
...
Now we have seen previously that a threshold is just the log odds ratio of responding in a category above vs below the threshold. Therefore, **a positive (negative) regression weight in an ordinal logistic regression should be interpreted as the log odds ratio to give a one level higher (lower) response**, i.e. R = 4 instead of R = 3 (R = 3 instead of R = 4).
...
To recapitulate that thresholds are just odds ratios at category boundaries, lets calculate them:
