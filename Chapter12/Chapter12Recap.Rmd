...
This is exactly our threshold and explains why the intercept coefficient of an intercept-only logistic regression for data with a fail-rate of 12% will be `qlogis(.12)` = `r round(qlogis(.12),1)`. The following figure shows that we can use the cumulative distribution function (`plogis` or `inv.logit`) to go from a threshold value to success probabilities and the quantile function (`qlogis` or `logit`) to go from cumulative success probabilities to thresholds.
