...
- Binomial: When the counted number of success is limited by the number of trials. This is the reason that the Binomial distribution has 2 parameters.
  - number of bad apples among all apples
...
set_par(mfrow = c(1,2))
table(data.list$failures) %>% barplot(border = "grey",   xlab = "number previous fails")
table(data.list$Medu) %>% barplot(border = "grey", ylab = "count", xlab = "maternal education")
```
...
axis(4,at = seq(0,0.4, length.out = 5),
     labels = seq(0,1,length.out = 5), col = "red")
...
- We had an odds ratio of 0.49, which corresponds to a log odds ratio of log(.49) ~ -.7. 
...
```{r class.source = 'fold-show', out.width="100%", fig.height = 3}
p = link_ulam(
  u.fit_F, post = prior, 
...
These differences and ratios are pretty large, so it is safe to make the prior a bit narrower and set the standard deviation for the regression weights to 1. [It is not surprising or wrong that positive differences are more likely, which is due to the prior on a].
...
One simple rule of thumb for logistic or binomial regressions is that with a $\small Normal(0,1)$ prior on regression coefficients the the 97.5 percentile is 1.96. If we exponentiate this we get 7, which means that a $\small Normal(0,1)$ prior expresses the prior information that the OR is probably not much larger then 7 or smaller than 1/7. Note that this prior would still be overwhelmed if we have a reasonable amount (N>25)^[I plugged this number from thin air, should really to a quick calculation on this.] of data.
...
Here is another example: If we want to see the difference in fail probability between maternal education levels 1 and 4, we calculate one vector of samples with fail probabilities at level 1, one for level 4, and subtract them:
...
The strong negative correlation reminds us that the Fylke intercept actually already represents the proportion of excused pupils in the first (any one of the) topics. 

While we can estimate such a model in a Bayesian way, the standard deviation of the correlated parameters will be inflated. 

We can solve this by re-formulating the model and use the indexing approach only one of the topics.
...
- The intercepts vary between -2.36 for Møre og Romsdal, i.e. the probability to be excused from the reading test is `exp(-2.36)` = `r round(exp(-2.36),2)`, 
- and -2.8 for Viken, i.e. the probability to be excused from the reading test is `exp(-2.8)` = `r round(exp(-2.8),2)`. 
- This means that the chance to be excused from the reading test is `exp(-2.36)/exp(-2.8) = `r round(exp(-2.36)/exp(-2.8),2)` larger in Møre og Romsdal, compared to Viken.
