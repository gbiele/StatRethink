...
We can talk about information in the model of the true data generating process and the candidate model, because we can calculate the probability of the data both for the true data generating process and the candidate model  (what we create with the `sim` function from rethinking packages).
...
mu = 1*bs2 + 1*bs2 + 1*bs3 + o*bs4
# I actually used "mu = bf.s %*% bf.b"
# where bf.s is a matrix with spline basis functions
# and bf.b is a vector with weights c(1,1,1,0)
p = dnorm(y, mu , 0.1)
```
...
We are calculating for each data point its average probability (averaging over posterior samples). In the following figure, each blue line is a prediction from the posterior distribution, and vertical lines represent the distance of observed data from predictions, where larger distances correspond to smaller probabilities of the data given the model. 
...
To complete the computation of the lppd we take the log of the just computed average probabilities. This results in a vector of log probabilities, one for each participant, which we can sum to get the a total score over all participants.

Given a quap model, the _log pointwise predictive density_ is simply calculated as:
