...
To identify which models is good, we need som measure or model accuracy, or consistency of model and data.

Imagine, you are working with a drug rehabilitation center and trying to evaluate different statistical models that predict if a patients will have a relapse within 4 weeks after leaving the center. Your data look as follows:
...
Multiplying probabilities and adding log probabilities leads to the same results that differ onøy due to numerical problems that arise when multiplying small probabilities:
...
Now lets look at the simple case of only two possible events. The following plot (bottom) shows on the x-axis the probability of the first event $\small p_1$ and on the y axis the entropy:
...
A related property is that if we start with two probabilities, e.g.
...
- We sample from the population data and estimate a model to infer a candidate DGP
- We want to estimate the accuracy of our model, i.e. the deviation between the true DGP and the infered a candidate DGP
...
plot(preds$x, preds$yhat,'l', col = "blue", ylab = "y", xlab = "x")
lines(preds$x, preds$y,'l', col = "red")
arrows(x0 = preds$x, y0 = preds$y, y1 = preds$yhat,length = .025, code = 3)
...
However, we generally do not know the true model, we only have a sample of data that was generated from this sample. This is not really a problem, because out initial goal was one of model comparison. So what we really want to do is to compare the deviances of to models $\small D_{KL}(p,q) - D_{KL}(p,r)$. Here is how the deviances are defined:

$$
D_{KL}(p,q)  = \sum_i p_i \,log(p_i) - p_i \, log(q_i) \\
D_{KL}(p,r)  = \sum_i p_i \,log(p_i) - p_i \, log(r_i)
$$
luckily, we can simplify this: Both deviances sum over $\small p_i \,log(p_i)$ and multiply the probability of the data with $\small p_i$. Therefore we can remove these terms and are then in the delightful situation that we only need to know about $\small log(q_i)$ and $\small log(r_i)$ if we want to compare candidate models!
