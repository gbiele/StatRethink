...
```{r alphaposterpred3, fig.width=7}
par(mfrow = c(1,2))
plot(dt,
     ylab = "weight",
     xlab = "length")
...
get_mu.model.pp = function(dt,posterior, type = "pred"){
  mu.model.pp = 
...
mu.model.pp = get_mu.model.pp(dt,mu.model.post)
...
```{r}
newdata = data.frame(a = 1,b = 2,length.s = seq(min(dt$length.s),max(dt$length.s),.1))
mu.model.pp.nd = 
  get_mu.model.pp(newdata,
                  mu.model.post)
...
plot(dt$length.s,
     ...
     xlab = "length",
     xaxt = "n")
conf_band(newdata$length.s,HDPI.lin_pred)
x.ticks = seq(35,60,by = 5)
axis(1, 
     at = (x.ticks-mean(dt$length))/sd(dt$length), 
     labels = x.ticks)
...
plot(dt$length.s,
     ...
     xlab = "length",
     xaxt = "n")
conf_band(newdata$length.s,HDPI.pp)
x.ticks = seq(35,60,by = 5)
axis(1, 
     at = (x.ticks-mean(dt$length))/sd(dt$length), 
     labels = x.ticks)
