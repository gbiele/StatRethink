...
```{r}
curve(dnorm(x,1.7,.1),1.3,2.1,
      ylab = "density")
d1.8 = dnorm(1.8, 1.7, 0.1)
lines(c(1.8,1.8,1),
      c(0,d1.8,d1.8), col = "red")
text(1.5,d1.8,round(d1.8,3),pos = 3, col = "red")
```
...
If we have samples and want to see how they are distributed, we can use a histogram or a density plot.
...
Another type of process that also leads to jointly distributed variables is Bayesian data analysis. If an analysis estimates several parameters, then we can look at posterior distribution of these parameters individually, we look at their marginal distributions, or we can look at the joint posterior distribution of the parameters. Even if I am just referring to posterior distributions of parameters here, we typically look at samples from this posterior distribution.
...
- Means ov the two marginal distributions (length and weight above)
- Variances (sd^2) for the two marginal distributions
- and also the co_variance of the two variables.
...
As you probably have guessed, covariance and correlation are closely related:
...
By looking at the range of the data, length varies roughly between around `r min.l` and `r max.l` and weight varies roughly between `r min.w` and `r max.w`. Then we can see that for length`r max.l` - `r min.l` = `r delta.l` and for weigh `r max.w` - `r min.w` = `r delta.w`. Finally we can calculate the `r delta.w` / `r delta.l` to see that for a one unit increase of length height should increase by around `r round(10*delta.w/delta.l)/10`.
...
One peculiar characteristic of this analysis is that the parameter values for $\alpha$ and $\beta$ are dependent. Remember that we discussed earlier that one can have dependency between any variables, which also includes parameter values, and that we can show this dependency by plotting both variables jointly in a (smooth) scatter plot.


Here is a smooth scatter plot of the parameters combinations for $\alpha$ and $\beta$ that have the highest posterior probability. We show these high posterior probability samples on the background of the parameter space we explored. Remember, Bayesian analysis tried to find parameter combinations are configurations that have a high posterior probability given the data, likelihood function, and priors. In the next figure the prior is in green and the posterior is in dark blue.
...
points(post$a,post$b, col = adjustcolor(tail(blues9,1),alpha = .5), pch = 16)
```
...
How is it possible that we see parameter combinations that have a similar posterior probability, and yet have vary different parameter values? To understand this, we can plot precictions, i.e. regression lines, from such parameter values.
...
To deal with these problems, some recommend to standardize predictor variables.

## Analysis with standardized predictor length

With standardization, we mean first and foremost to subtract the mean from the predictor variable, as is done in the next plot. One could also divide by the standard deviation, but then the interpretation of slope parameters is less intuitive.
...
for (k in 1:50)
  abline(rnorm(1,3.5,1.5), rnorm(1,.15,.5), col = clr)
```
...
abline(v = 0, lty = 3, col = "grey")
for (k in 1:50)
  abline(rnorm(1,3.5,1), rlnorm(1,log(.175),.5), col = clr)
```
...
|Prior | $\beta \sim logNormal(log(.15), .5)$ | `beta ~ dlnorm(log(.15), .5)` |
|Prior | $\sigma \sim Uniform(0,1.5)$ | `sigma ~ dunif(0, 1.5)` |
...
We found that a on standard deviation increases in birth length was associated with a `r round(mean(mu.model.post$b)*1000)` gram (credible interval `r paste(round(HPDI(mu.model.post$b*1000)), collapse = ", ")`) increase of birth weight.
...
matplot(B,type = 'l', main = "5 knots")
num_knots = 12
knot_list = quantile(df$x,probs=seq(0,1,length.out=num_knots))
B = bs(df$x, knots=knot_list, degree=3)
matplot(B,type = 'l', main = "12 knots")
```


Another thing we can decide is how large we allow weights $\omega$ for the Basis functions become.

To understand this, we instepct this quap model:
...
```{r}
B %*% matrix(rnorm(ncol(B)*5,sd = 1),nrow = ncol(B)) %>% matplot(type = 'l', col = "red", lty = 1)
B %*% matrix(rnorm(ncol(B)*5,sd = .1),nrow = ncol(B)) %>% matlines(col = "blue", lty = 1)
legend("topright",
       col = c("red","blue"),
       lty = 1,
...
```
The last figure here shows the influence of number of knots (in rows) and the standard deviation for omega (in columns).

```{r}
par(mfrow = c(2,2))
for (n_knots in c(5,15)) {
  for(sd in c(0.1,1)) {
    knot_list = quantile(df$x,probs=seq(0,1,length.out=n_knots))
    B = bs(df$x, knots=knot_list, degree=3)
    B %*% matrix(rnorm(ncol(B)*5,sd = sd),nrow = ncol(B)) %>% 
      matplot(type = 'l', col = "blue", lty = 1, ylim = c(-2,2))
