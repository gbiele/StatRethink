...
for (k in 1:length(small_a.a)) abline(small_a.a[k],small_a.b[k], col = adjustcolor("red",alpha = .125))
for (k in 1:length(large_a.a)) abline(large_a.a[k],large_a.b[k], col = adjustcolor("blue",alpha = .125))
```

Parameters are hard to interpret. What do intercept $\alpha$ and slope $\beta$ mean intuitively?
...
plot(0,
     ...
     xlim = c(-11,11))
for (k in 1:50)
  abline(rnorm(1,3.5,1), rlnorm(1,log(.15),.5), col = clr)
```
