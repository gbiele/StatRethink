...
text(40,1,expression(w[i]), pos = 4, col = "blue", cex = 2, font = 3)
par(mar=c(5,2,0,0))
...
text(-2,1900,expression(sigma~"="~Uniform(3.5,1.5)))
```
...
text(-3300,1400,expression(sigma~"="~Uniform(0,10000)))
```
...
```{r}
mu.model.post = extract.samples(mu.model.fit,n=1e4)
head(mu.model.post)
```
...
```{r}
hist(mu.model.post$mu, main = "",xlab = expression(mu))
abline(v = HPDI(mu.model.post$mu), col = "blue")
...
```{r}
pp.weight = rnorm(250,
      mu.model.post$mu[1:250],
      mu.model.post$sigma[1:250])
