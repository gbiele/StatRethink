...
hist(prior.pedictive.weights, main = "",
     ...
     col = my_blue)
text(-4500,1500,expression(mu~"="~Normal(0,10000)), pos = 2)
text(-4500,1350,expression(sigma~"="~Uniform(0,10000)), pos = 2)
```
...
for (k in 1:50) abline(a = rnorm(1,-1.5,3), b = rnorm(1,.1,.2), col = clr)
```
...
large_a.idx = which(post$a >= sort(post$a,decreasing = T)[25])
...
points(small_a.a, small_a.b,col = "red", pch = 16)
points(large_a.a, large_a.b,col = "blue", pch = 16)
```
...
with(post,
     for (k in 1:nrow(post)) {
       mu = a[k] + b[k] * dt$length # lineqar predictor
       lp__[k] = 
         sum(dnorm(dt$weight, mu, sigma[k],log = T)) + 
         dnorm(a[k],-1.5,3,log = T) + 
         dnorm(b[k],0.1,.1,log = T) + 
         dunif(sigma[k],0,1.5,log = T)
     }
)
plot(post$a,lp__,pch = 16, col = adjustcolor("blue",alpha = .1))
abline(v = mean(post$a))
plot(post$b,lp__,pch = 16, col = adjustcolor("blue",alpha = .1))
abline(v = mean(post$b))
plot(post$sigma,lp__,pch = 16, col = adjustcolor("blue",alpha = .1))
```


```{r}
plot(dt$length,dt$weight, xlim = c(0,65), ylim = c(-3,9))
abline(a = mean(post$a), b = mean(post$b), lwd = 2)
for (k in 1:length(small_a.a)) abline(small_a.a[k],small_a.b[k], col = adjustcolor("red",alpha = .25))
for (k in 1:length(large_a.a)) abline(large_a.a[k],large_a.b[k], col = adjustcolor("blue",alpha = .25))
```
...
text(2,coeffs[1] + 2*coeffs[2], expression(mu~"="~alpha+beta*X), pos = 1, col = "blue", cex = 1.5, srt=25.6)
```
...
We found that a on standard deviation increases in birth length was associated with a `r round(mean(mu.model.post$b)*1000)` (credible interval `r paste(round(HPDI(mu.model.post$b*1000)), collapse = ", ")`) gram increase of birth weight.
