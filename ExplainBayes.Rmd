...
```{r Filter, fig.cap = "Histogram of samples obtained by simulating from prior knowledge colored by if the number of successes simulated with a sample is equal to the observed number of successes."}
# filter
filtered.samples = 
...
```{r PriorFilterPosterio, fig.cap = "Histogram of samples obtained by simulating from prior knowledge, colored after filtering and retained after filtering.", fig.width=11}
par(mfrow = c(1,3))
hist(info_prior,
     main = "Prior",
     xlab = "theta")
...
parameter_table = rbind(
  parameter_table,
  parameter_table %>% 
    summarise(Source = "Prior & data",
              Prob.success = weighted.mean(Prob.success, Observations) %>% round(3),
...
```{r Experiment, fig.cap = "Bayesian updating in one figure."}
