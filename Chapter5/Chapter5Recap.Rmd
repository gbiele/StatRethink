...
Which regression models can you estimate to test dag3?
...
Next we use the `link` function from the `rethinking` package to calculate values $\small W$ for varying $\small A$ while we fix $\small S = 0$. We plot counterfactual effect with a solid line and add for comparison the effect from the unadjusted model (`m.A`) with a dotted line.
