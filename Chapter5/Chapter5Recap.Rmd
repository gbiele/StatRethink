...
|Prior | $\sigma \sim Exponential(1)$ | `sigma ~ dexp(1)` |
...
So, if we disregard our knowledge about daily reading duration of parents, we conclude that there is a strong relationship between the number of books in a household and child reading ability. If one would be willing to interpret the association as causal, one could even think that the number of books in a household is an important cause of child reading ability.
...
for (i in idx) 
  lines(rep(d$C[i],2),
        mu_PI[,i],
        col="blue")
title("linear predictions + credible intervals", cex.main = 1)
```
...
Counterfactual plots tell us about the effect of an intervention, under the condition that we assume or have identified the correct data generating process in our causal model (which may be represented as a DAG).
...
```{r class.source = 'fold-show'}
D_seq  = seq(from=-2, to=2, length.out=20)
```
...
This effect, represented by the slope, is a large. Next we calculate by how many standard deviations child reading gets better if we increase daily reading of parents from 0 to 1 (which is also a 1 sd increase because the sd of parental reading is 1.).
