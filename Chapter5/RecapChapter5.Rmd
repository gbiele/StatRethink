...
![Understanding residuals](residuals.mp4)
...
But we already new that this DAG was not correct. Now lets check the conditional indendencies of the true DAG:
...
drawdag(ME.dags)
mtext("1", outer = T, line = -1,adj = .15)
mtext("2", outer = T, line = -1,adj = .5)
mtext("3", outer = T, line = -1,adj = .85)
mtext("4", outer = T, line = -15,adj = .15)
mtext("5", outer = T, line = -15,adj = .5)
mtext("6", outer = T, line = -15,adj = .85)
```

The important insight here is that if we only observe a data set where $\small A$, $\small S$,  and $\small W$ are connected and no conditional independencies are present, we cannot infer which of the 6 DAGs shown in the previous figure generated the data. If we still want to make some progress, we need use domain knowledge to roul out a DAGs.
