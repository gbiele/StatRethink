...
No we can use $Br$ as a predictor of $\small C$ and plot the regression line:
...
We can visualize this correlation in a scatter plot. Because we do not expect that the prediction exactly coincide the observed values, it us useful to add credible of HDPIs to the predicted values. Then one can check if those overlap with the regression line that visualizes perfect coincidence of observed and predicted data.
...
If the main concern is if the observed data are within the most plausible predicted data, one should use credible intervals from posterior predictions. If one has for example 90% credible interval, 90% of the observed value should be within the credible interval of the predicted values.

If the main concern is to detect systematic is strong deviations between observed and predicted data, these van be easier seen if one used credible intervals from the linear predictions.
...
In this example, there are no strong deviations between predicted and observed values. After all, the regression is based on the true model. Here are the same types of plots if the true DGP is a non-linear effect $\small D$ on $\small C$, but ourt model assumes only a linear relationship:
...
- Set the manipulated variable to chosen values. This makes the manipulated variables independent of all other variables in the DAG/model, it makes it "exogenous"
- Using coefficients from you model, calculate all down stream effect of manipulate variables. This includes direct and indirect effects on the outcome of interest.
...
If we want to manipulate $\small D$, we have to calculate the resulting direct effect $\small D \rightarrow C$ and the indirect effect $\small D \rightarrow B \rightarrow C$. In order to do this, we need a regression coeffcients for all these paths. With `quap` we can estimate them in one model:
...
This is a large effect. Next we calculate by how many sd child reading gets better if we go from increase daily reading of parents from 0 to 1 (which is also an sd of 1 because the sd of parental reading is 1.).
...
Now we look at counterfactuals for B, which we show to the left of the counterfactuals for D. We calculate the effect of B while we fix D to zero. ^[Fixing D to a specific value is OK here because this is a simple linear regression without interactions. Things get more complicated when interactions play a role or non-linear link functions are used, like e.g. for logistic regressions.]
...
And we plot the simulated data.
```{r fig.height=5, fig.width=10}
par(mfrow = c(1,2))
plot(sim_dat$D,colMeans(s$C),
...
As expected, this effect is much weaker.
