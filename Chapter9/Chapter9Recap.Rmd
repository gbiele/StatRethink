...
To start the sampling process, we set some initial parameter values. We store the initial values in vectors that will also hold our posterior samples. The vector `P` holds the log posteriors.
...
- If the log posterior of the proposal is lower, randomly choose between the last sample and the new sample. The log posteriors determine the probability of choosing the proposal or old sample, such that the smaller the log posterior of the proposal is compared to the log posterior of the last sample, the smaller is the probability to choose the proposal. 
...
So the probability of the proposal is `r round(exp(proposal.P-P[1]),2)` time the probability of the last sample.
...
The effective sample size (n_eff or ess) can be understood as the number of independent samples in the posterior. It is a direction function of Rhat and the number of posterior samples. One should not just increase the number of iterations when the effective sample size is low. Rather, one should check if the model can be improved through better priors and/or a re-parameterization.
...
- Typically 4 chains are sufficient. Fewer chains reduce the chance to detect problems. More chains can be useful to detect problems in tricky models. (this would typically be )
...
The reasoning is that modern samplers like Stan's Hamiltonian Monte Carlo NUTS sampler are so goodd that they can fit almost any model, and that therefore problems during model estimation are not due to the sampler but due to a model that is not specifies well.

One important part of model specification are the priors.

To see this, lets look at the problem of colinearity and specify and extremely co-linear situation, where we use two times the same parameter to predict an outcome:
...
If we specify a model $\small y ~ normal(a + \beta_1 X_1 + \beta_2 X_2, \sigma)$:
...
then this is non-identified because e.g. the prediction for $\small  \beta_1 = 1, beta_2 = 0$, $\small  \beta_1 = 0, beta_2 = 1$, $\small \beta_1 = 0.5, beta_2 = 0.5$, $\small \beta_1 = 0.75, beta_2 = .25$ will be the same. In fact there is an infinite number of combinations of values for $\small \beta_1$ and $\small \beta_2$ that make identical predictions.
...
This model did clearly not work out. Lets look again and the model and think about how we could improve the model. We start with prior preditions: 

```{r, class.source = 'fold-show', results='hide', message=FALSE}
prior = extract_prior_ulam(u.fit.wide)
mu = link_ulam(u.fit.wide, data = list(X1 = range(data.list$X1)), post = prior)
```
...
We see that prior for beta allows slope that are orders of magnitude steeper than is reasonable. Lets look at the priors again:

```{r}
wide.model
```

The priors on b1 and b2 are indeed very wide. Note that a 4 unit change on the x axis corresponds to a 12 unit change on the y axis, for only one parameter. 12/4/2 = 1.5, so even if we set the prior sd for b1 and b2 to 15, we are stilling using very wide priors (though not ridiculously  wide). Lets try this model:
...
u.fit.less.wide = ulam(
  ...
  cores = 4,        # use 4 cores in parallel
  cmdstan = TRUE)   # use cmdstanr not rstan
traceplot(u.fit.less.wide, max_rows = 1, pars = c("b1","b2"), n_cols = 2)
```
