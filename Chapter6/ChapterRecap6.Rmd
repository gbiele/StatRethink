...
To obtain a correct estimate of the relationship between $\small B$ and $\small D$, we have to cut off the back door path.
...
An alternative view point, which only works if the confounder is a grouping variable, is that adjustment amount to _condition_ the association between $\small B$ and $\small D$ on sex $\small S$, that is we estimate the association for the two sex groups separately:
...
The effect of causes can be (partially or fully) mediated by auxiliary variables. Specifically, if we have the following _pipe_ structure the effect of $\small Y$ on $\small X$ is fully mediated my $\small M$ because there is no other path from $\small Y$ to $\small X$ tat does not go via $\small M$. 

In experimental studies, post-treatment variables are variables that manifest after the treatment was delivered. Post-treatment have a higher likelihood to be mediators.
...
- treatment as a direct effect on wellbeing, but also 
...
- Why can you get from X to Y through other variables with opposite direction arrows?
- Can the outcome variable be a collider
- Is the selection bias in the newsworthiness trustworhtiness example the collider?
- McElreaths point out that multicollinearity only concerns correlated variables in the model. In other texts we have been recommended to look at bivariate correlations between predictors to examine potential multicollineraity. What is the best way to find potential multicollinerarity? Are there any alternatives than the method described by McElreath (examine the model with and without both (or several) predictors)?
- Isn’t the post-treatment effect just a special case of multicollinearity or is there some difference we did not catch?