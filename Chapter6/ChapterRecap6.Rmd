...
The effect of causes can be (partially or fully) mediated by auxiliary variables. Specifically, if we have the following _pipe_ structure the effect of $\small Y$ on $\small X$ is fully mediated my $\small M$ becuase there is no other path from $\small Y$ to $\small X$ tat does not go via $\small M$.
...
In contrast to bias from adjustment with a mediator or bis from confounding, selection bias cannot be solved by adding or removing an adjustment variable. To deal with selection bias, methods based on propensity scores^[propensity score methods use auxiliary variables to calculate the probability that a participant is included in a sample or receives a treatment], like matching or weighting need to be used. Note tough that propensity core methods and especially weighting do not work naturally (some would say at all) in the Bayesian framework.
